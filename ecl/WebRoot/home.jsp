...
<body>
		...
		<div class="top_nav">
			...
			<div class="nav_width">
				...
				<ul class="nav navbar-nav pull-right">
					...
					<li class="nav-item"><a class="nav-link"
						href="<c:url value='/SharedServlet?method=myShared' />">我的分享</a>
					</li>

					<li class="nav-item"><a class="nav-link"
						href="<c:url value='/SharedServlet?method=myShared' />">公共分享</a>
					</li>
					<li class="nav-item"><a class="nav-link" href='<c:url value="/UserServlet?method=quit"></c:url>'>退出登录</a>
					</li>
				</ul>
			</div>
			</nav>
				...
				<div class="row">
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
						<div class="admin">
							...
							<p>${sessionScope.user.userName}:欢迎你</p>
							<table class="table">
								<p>${msg }</p>
							</table>
						</div>
						...
						<div class="rightarea">
							<div class="righthead">
								...
								<button onclick="upload()" type="button"
									...
									<span><i
										class="glyphicon glyphicon glyphicon-circle-arrow-up"></i>
									</span> 上传文件
								</button>
									...
									<c:otherwise>
										...
										<c:forEach items="${catalog.children }" var="c">
											<tr>
												...
												<td></td>
												<td><a
													href='<c:url value="/CatalogServlet?method=deleteCatalog&cid=${c.cId }&pid=${c.parent.cId }"></c:url>'>删除</a><a>重命名</a>
												</td>
											</tr>
										</c:forEach>
										...
										<c:forEach items="${catalog.myFile }" var="f">
											<tr class="${f.fId } abc">
												...
												<td>${f.fType }</td>
												<td>${f.fDowncount }</td>
												<td jq="true"><a href="#" iid=${f.fId }>删除</a><a>重命名</a>
													<a
													href='<c:url value="/DownLoadServlet?fid=${f.fId }&cid=${c.cId }"></c:url>'>下载</a>
												</td>
												<!-- 											</tr> -->
										</c:forEach>
	...
	<form id="uploadform" action="<c:url value='UploadServlet' />"
		...
		<input type="file" name="filename" onChange="fileName()" /><br /> <input
			type="hidden" name="cid" value="${catalog.cId }" /> <input
